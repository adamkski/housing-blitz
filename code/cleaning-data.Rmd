...
ads <- stream_in(file(r"(C:\Users\kowal\Documents\projects\housing\housing-scrape_craigslist_2020-07-25.jl)")) %>% 
  as_tibble %>% 
  rowid_to_column("id") 
...
add_parking <- 
  attributes %>% 
  filter(rental_attr %in% c("attached garage", "off-street parking", "no parking", "street parking", "carport")) %>% 
...
attributes %>%
  filter(!str_detect(rental_attr, "BR"),
         ...
         !rental_attr %in% c("attached garage", "off-street parking", "no parking", "street parking", "carport"),
...
ads %>% 
  ...
  write_csv(here("data/raw-housing-scrape_craigslist_2020-07-25.csv"))

ads_renamed %>% 
  select(-raw_description, everything(), raw_description) %>% 
  write_xlsx(here("data/structured-housing-scrape_craigslist_2020-08-08.xlsx"))
```
